...
(function (global, factory) {
  ...
  var DEFAULTS = {
    ...
    // ADDED FOR MIN/MAX YEAR
    minYear: null, // If null, no enforced minimum
    maxYear: null, // If null, no enforced maximum
  ...
  var helpers = {
    render: function render(type) {
      ...
      // Only calculate base for non-year types
      // so that hours or months continue to wrap
      // but years do not.
      if (type !== "year" && isFinite(max)) {
      ...
      for (var i = 0; i < options.rows + 2; i += 1) {
        ...
        // Only do the modulo-based wrap for non-year columns
        if (type !== "year" && base) {
          newValue %= base;
          if (newValue < min) {
            newValue += base;
                ...
                switch (token.charAt(0)) {
                  case "Y":
                    ...
                    if (typeof options.minYear === "number") {
                      data.min = Math.max(data.min, options.minYear);
                    }

                    // If user specified a maxYear, override the "max"
                    if (typeof options.maxYear === "number") {
                      data.max = Math.min(data.max, options.maxYear);
                    }
