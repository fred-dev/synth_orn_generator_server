...
map.on("contextmenu", async function (event) {
  ...
  flatpickr("#date", {
    ...
    onChange: function (selectedDates, dateStr, instance) {
      ...
      const day_of_year = getDayOfYear(selectedDates[0]);
      ...
      fillSuggestedWeatherData(selectedDates[0]);
    },
    onClose: function (selectedDates, dateStr, instance) {
      ...
      const minutes_of_day = new Date(dateStr).getHours() * 60 + new Date(dateStr).getMinutes();
      userGeneratedData.minutes_of_day = minutes_of_day;
      //fetch the popup so we can change the content
  ...
  `;
      ...
      document.getElementById("confirmBtn").addEventListener("click", async () => {
        //close the popup
        marker.closePopup();
        console.log("All conditions confirmed preparing request for text and audio");
  ...
  async function fillSuggestedWeatherData(selectedDate) {
    ...
    latlng.let = userGeneratedData.lat;
    latlng.lng = userGeneratedData.lon;;
