...
import { saveAudioAndData, savePromptToFile } from "../utils/fileUtils.js";
import { 
  validateCoordinates, 
...
// Global variable to store user input for audio generation
let userInputData = {};

router.post("/generateAudio", async (req, res) => {
  try {
    ...
    const weatherParams = validateWeatherParams(temp, humidity, wind_speed, pressure);
    const dateParams = validateDateParams(minutes_of_day, day_of_year);
    
    logger.debug("Received audio generation request:", { coords, weatherParams, dateParams });
    
    const result = await gradioService.generateAudio({
      lat: coords.lat,
      lon: coords.lon,
      temp: weatherParams.temp,
      humidity: weatherParams.humidity,
      wind_speed: weatherParams.windSpeed,
      pressure: weatherParams.pressure,
      minutes_of_day: dateParams.minutesOfDay,
      day_of_year: dateParams.dayOfYear,
    });
    
    res.json({ audioUrl: result });
    
    // Save audio and data asynchronously
    saveAudioAndData(result, userInputData).catch(error => {
      logger.error("Error saving audio and data:", error);
    });
    
  } catch (error) {
    logger.error("Error in generateAudio:", error);
...
router.post("/generate-text", async (req, res) => {
  try {
    ...
    validateRequiredFields(userInput, ['date', 'locationName', 'temperature', 'humidity', 'windSpeed', 'waterDistance']);
    ...
    await openaiService.generateText(prompt, res);
    
  } catch (error) {
    logger.error("Error in generate-text:", error);
    if (!res.headersSent) {
...
router.get("/weather", async (req, res) => {
  try {
    ...
    validateRequiredFields({ lat, lon, date }, ['lat', 'lon', 'date']);
    validateCoordinates(lat, lon);
    
    logger.debug("Weather request:", { lat, lon, date });
    
    const data = await weatherService.getHistoricalWeather(lat, lon, date);
    res.json(data);
...
router.post("/waterdistance", async (req, res) => {
  try {
    const { lat, lon } = req.body;
    
    validateRequiredFields({ lat, lon }, ['lat', 'lon']);
    validateCoordinates(lat, lon);
    
    logger.debug("Water distance request:", { lat, lon });
    
    const data = await pythonService.getWaterDistance(lat, lon);
    res.json(data);
    
  } catch (error) {
    logger.error("Error in waterdistance endpoint:", error);
    res.status(500).json({ error: error.message });
  }
});

router.post("/isInAustralia", async (req, res) => {
  try {
    const { lat, lon } = req.body;
    ...
    logger.debug("isInAustralia request:", { lat, lon });
    
    const data = await pythonService.checkIsInAustralia(lat, lon);
    res.json(data);
    
  } catch (error) {
    logger.error("Error in isInAustralia endpoint:", error);
    res.status(500).json({ error: error.message });
  }
});

router.post("/hug_space_control", async (req, res) => {
  try {
    const { command } = req.body;
    
    validateRequiredFields({ command }, ['command']);
    
    logger.debug("Hug space control request:", { command });
    
    const data = await pythonService.controlHugSpace(command);
    res.json(data);
    
  } catch (error) {
    logger.error("Error in hug_space_control endpoint:", error);
    res.status(500).json({ error: error.message });
  }
});
